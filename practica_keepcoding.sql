...
-- Creo tablas
create table flota_keepcoding.vehiculo(
...
create table flota_keepcoding.grupo_empresarial(
	id serial primary key,
...
create table flota_keepcoding.revision(
	id serial primary key,
	revision_kilometros INT NOT NULL,
...
-- Creo las relaciones
alter table flota_keepcoding.vehiculo add constraint pk_modelo_vehiculo foreign key (id_modelo) references flota_keepcoding.modelo(id);
alter table flota_keepcoding.modelo add constraint pk_marca_vehiculo foreign key (marca_id) references flota_keepcoding.marca(id);
alter table flota_keepcoding.marca add constraint pk_grupo_empresarial foreign key (grupo_empresarial_id) references flota_keepcoding.grupo_empresarial(id);
alter table flota_keepcoding.revision add constraint pk_revision_moneda foreign key (moneda_id) references flota_keepcoding.moneda(id);
alter table flota_keepcoding.revision add constraint pk_revision_vehiculo foreign key (matricula_vehiculo) references flota_keepcoding.vehiculo(matricula);
alter table flota_keepcoding.poliza add constraint pk_poliza_aseguradora foreign key (aseguradora_id) references flota_keepcoding.aseguradora(id);
alter table flota_keepcoding.poliza add constraint pk_poliza_vehiculo foreign key (matricula_vehiculo) references flota_keepcoding.vehiculo(matricula);

-- Creo la tabla temporal para importar datos

CREATE TABLE flota_keepcoding.coches (
...
-- Traigo datos de tabla temporal a mis tablas principales

insert into flota_keepcoding.grupo_empresarial (nombre) 
select grupo from flota_keepcoding.coches group by grupo;

select * from flota_keepcoding.grupo_empresarial ge 


insert into flota_keepcoding.marca (nombre, grupo_empresarial_id)
select 
	c.marca,
	ge.id 
from flota_keepcoding.coches c
inner join flota_keepcoding.grupo_empresarial ge on c.grupo = ge.nombre
group by c.marca, ge.id

select * from flota_keepcoding.marca m 


insert into flota_keepcoding.modelo (nombre, marca_id)
select 
	c.modelo,
	marca.id
from flota_keepcoding.coches c
inner join flota_keepcoding.marca on c.marca = marca.nombre
group by c.modelo, marca.id

select * from flota_keepcoding.modelo m 


insert into flota_keepcoding.vehiculo (matricula, color, total_kilometros, fecha_compra, id_modelo)
select
	c.matricula,
	c.color,
	c.kms_totales,
	...
	modelo.id  
from flota_keepcoding.coches c
inner join flota_keepcoding.modelo on c.modelo = modelo.nombre 
group by c.matricula, c.color, c.kms_totales, c.fecha_compra, modelo.id
order by c.matricula 
	
select * from flota_keepcoding.vehiculo v 



insert into flota_keepcoding.aseguradora (nombre)
select aseguradora from flota_keepcoding.coches c group by aseguradora;


insert into flota_keepcoding.poliza (numero_poliza, fecha_alta, aseguradora_id, matricula_vehiculo)
select
	c.n_poliza,
	c.fecha_alta_seguro,
	a.id,
	v.matricula
from flota_keepcoding.coches c 
inner join flota_keepcoding.aseguradora a on c.aseguradora = a.nombre
inner join flota_keepcoding.vehiculo v on c.matricula = v.matricula 
group by c.n_poliza, c.fecha_alta_seguro, a.id, v.matricula
order by v.matricula 

select * from flota_keepcoding.poliza p 



insert into flota_keepcoding.moneda (nombre)
select moneda from flota_keepcoding.coches c group by moneda;


insert into flota_keepcoding.revision (revision_kilometros, revision_fecha, importe, moneda_id, matricula_vehiculo)
select
	c.kms_revision,
	c.fecha_revision,
	c.importe_revision,
	...
	v.matricula
from flota_keepcoding.coches c
inner join flota_keepcoding.moneda m on c.moneda = m.nombre
inner join flota_keepcoding.vehiculo v on c.matricula = v.matricula
group by c.kms_revision, c.fecha_revision, c.importe_revision, m.id, v.matricula
order by v.matricula

select * from flota_keepcoding.revision r
...
from flota_keepcoding.vehiculo v 
inner join flota_keepcoding.modelo m on v.id_modelo = m.id
inner join flota_keepcoding.marca m2 on m.marca_id = m2.id 
inner join flota_keepcoding.grupo_empresarial ge on m2.grupo_empresarial_id = ge.id
inner join flota_keepcoding.poliza p on v.matricula = p.matricula_vehiculo 
inner join flota_keepcoding.aseguradora a on p.aseguradora_id = a.id
group by 
	m.nombre, 
	m2.nombre , 
	ge.nombre , 
